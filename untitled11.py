...
st.title("🌍 Delhi Air Quality Prediction & Monitoring System")
st.write("Select any Delhi area to view AQI prediction, health alerts, and movement recommendations.")

# -------------------------------------------------------------
# ✔ MANUAL DATA (EXTRACTED FROM YOUR final_converted.csv)
# -------------------------------------------------------------
data = {
    "Place": [
...
# ✔ SELECT LOCATION
# -------------------------------------------------------------
place = st.selectbox("📌 Select a Delhi Location", df["Place"].unique())
row = df[df["Place"] == place].iloc[0]
aqi = row["Predicted_AQI"]

st.subheader(f"📍 Air Quality Details for **{place}**")

# -------------------------------------------------------------
# ✔ WEATHER & POLLUTANT VALUES
# -------------------------------------------------------------
st.write("### 🌡 Weather & Pollutant Levels")
...
def best_time(aqi):
    if aqi <= 80:
        return "🟢 Morning 6 AM – 10 AM (Fresh air)"
    elif aqi <= 150:
        return "🟡 Evening 5 PM – 8 PM"
    elif aqi <= 250:
        return "🟠 9 PM – 5 AM (Avoid smog hours)"
    else:
        return "🔴 Avoid going outside today"

# -------------------------------------------------------------
# ✔ FUNCTION: Vehicle Recommendation
# -------------------------------------------------------------
...
# -------------------------------------------------------------
# ⭐ WOW FACTOR — AI ALERT SYSTEM FOR SCHOOLS, HOSPITALS, SENIOR CITIZENS
# -------------------------------------------------------------
def school_alert(aqi):
    if aqi <= 150:
        return "🟢 Schools can operate normally."
    elif aqi <= 250:
        return "🟠 Schools should avoid morning assemblies."
    else:
        return "🔴 Schools advised to shift to **online classes**."

def hospital_alert(aqi):
    if aqi <= 120:
        return "🟢 Safe for normal patients."
    elif aqi <= 200:
        return "🟡 Asthma/COPD patients should limit outdoor movement."
    else:
        return "🔴 Hospitals should issue **respiratory emergency warnings**."

def senior_alert(aqi):
    if aqi <= 120:
        return "🟢 Elders (60+) can move outside carefully."
    elif aqi <= 200:
        return "🟡 Morning/Evening walks should be avoided."
    else:
        return "🔴 Seniors should stay indoors; high risk AQI."
...
st.subheader("🏫 School Safety Alert")
st.warning(school_alert(aqi))

st.subheader("🏥 Hospital & Patient Alert")
st.error(hospital_alert(aqi))

st.subheader("👴 Senior Citizen Alert")
st.info(senior_alert(aqi))

# -------------------------------------------------------------
# ✔ PLOT: Actual vs Predicted AQI
# -------------------------------------------------------------
st.header("📈 Actual vs Predicted AQI Across Delhi")
